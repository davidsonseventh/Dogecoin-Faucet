...
if($user){
	...
	if(time() < $nextClaim){
		header("Location: https://ouo.io/M2Ws1D9");
		exit;
	}
	...
	if(isset($_POST['verifykey'])){
		if($_POST['verifykey'] != $user['claim_cryptokey']){
			$content .= alert("danger", "Claim failed. <a href='https://ouo.io/M2Ws1D9'>Go back</a>");
		} else {
		...
		$content .= alert("danger", "Abusing the system is not allowed. <a href='https://ouo.io/M2Ws1D9'>Go back</a>");
	}
} else {
	header("Location: https://ouo.io/M2Ws1D9");
	exit;
