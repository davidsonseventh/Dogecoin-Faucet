...
class FaucetPay
{
    ...
    public function __construct($api_key, $currency = "Doge", $disable_curl = false, $verify_peer = true, $timeout = null) {
    ...
    public function send($to, $amount, $referral = false, $ip_address = "") {
        ...
        if (array_key_exists("status", $r) && $r["status"] == 456) {
            return array(
                'success' => false,
                'message' => $r['message'],
                'html' => '<div class="alert alert-danger">Before you can receive payments at FaucetPay.io with this address you must link it to an account. <a href="https://faucetpay.io/?r=366233" target="_blank">Create an account at FaucetPay.io</a> and link your address, then come back and claim again.</div>',
                'response' => json_encode($r)
            );
        }
